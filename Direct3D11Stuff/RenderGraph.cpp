...
namespace Rgph
{
	RenderGraph::RenderGraph(Graphics& gfx) : backBufferTarget(gfx.GetTarget()), masterDepth(std::make_shared<Bind::OutputOnlyDepthStencil>(gfx))
	{
	...
	{
		const auto finder = [&sinkName](const std::unique_ptr<Sink>& p) { return p->GetRegisteredName() == sinkName; };
	...
	{
		const auto i = std::find_if(passes.begin(), passes.end(), [&name](auto& p) { return p->GetName() == name; });
		if (i == passes.end())
		{
