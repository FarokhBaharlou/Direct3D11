...
namespace Rgph
{
	...
	public:
		...
		ShadowMappingPass(Graphics& gfx, std::string name) : RenderQueuePass(std::move(name))
		{
			...
			DirectX::XMStoreFloat4x4(&projection, DirectX::XMMatrixPerspectiveFovLH(PI / 2.0f, 1.0f, 0.5f, 100.0f));
			// +x
		...
		void Execute(Graphics& gfx) const noxnd override
		{
			...
			auto tempCamPos = pShadowCamera->GetPos();
			const auto pos = XMLoadFloat3(&tempCamPos);
