...
namespace Dcb
{
	...
	ConstElementRef::ConstElementRef(const LayoutElement* pLayout, const char* pBytes, size_t offset) noexcept : offset(offset), pLayout(pLayout), pBytes(pBytes) {}
	...
	ElementRef::ElementRef(const LayoutElement* pLayout, char* pBytes, size_t offset) noexcept : offset(offset), pLayout(pLayout), pBytes(pBytes) {}
	...
	Buffer::Buffer(const CookedLayout& lay) noxnd : pLayoutRoot(lay.ShareRoot()), bytes(pLayoutRoot->GetOffsetEnd()) {}
	Buffer::Buffer(CookedLayout&& lay) noxnd : pLayoutRoot(lay.RelinquishRoot()), bytes(pLayoutRoot->GetOffsetEnd()) {}
	Buffer::Buffer(const Buffer& buf) noexcept : pLayoutRoot(buf.pLayoutRoot), bytes(buf.bytes) {}
	Buffer::Buffer(Buffer&& buf) noexcept : pLayoutRoot(std::move(buf.pLayoutRoot)), bytes(std::move(buf.bytes)) {}
