...
namespace Rgph
{
	...
	public:
		...
		DirectBufferSource(std::string name, std::shared_ptr<T>& buffer) : Source(std::move(name)), buffer(buffer) {}
	...
	public:
		...
		DirectBindableSource(std::string name, std::shared_ptr<T>& bind) : Source(std::move(name)), bind(bind) {}
		void PostLinkValidate() const {}
